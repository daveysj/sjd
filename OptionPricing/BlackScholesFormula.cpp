...
namespace sjd {
   ...
   BlackInputs
    =======================================================================================*/
    BlackInputs::BlackInputs() : F(0), X(0), yte(0), yts(0), sd(0), df(0)
    {
        errorMessage = "BlackInputs->Error checking has not happened, but all inputs set to zero";
    }

    BlackInputs::BlackInputs(double forward, double strike, double yearsToExpiry, double yearsToSettlement, 
                             double standardDeviation, double discountFactor) :
                F(forward), X(strike), yte(yearsToExpiry), yts(yearsToSettlement), 
                sd(standardDeviation), df(discountFactor), errorMessage("Not tested")
    {
        errorMessage = "BlackInputs->Error checking has not happened, please call isOK() first to check for errors in input parameters";            
    }

    void BlackInputs::setValues(double forward, double strike, double yearsToExpiry, double yearsToSettlement, 
                                double standardDeviation, double discountFactor)
    {
        F = forward;
        X = strike;
        yte = yearsToExpiry;
        yts = yearsToSettlement;
        sd = standardDeviation;
        df = discountFactor;
        errorMessage =  "BlackInputs->Error checking has not happened, please call isOK() first to check for errors in input parameters";
    }

    bool BlackInputs::isOK()
    {
        if (F <= 0) {
            errorMessage = "BlackInputs->Forward is <= 0";        
            return false;
        }
        if (X <= 0) {
            errorMessage = "BlackInputs->Strike is <= 0";        
            return false;
        }
        if (yte <= 0) {
            errorMessage = "BlackInputs->Years to expiry are <= 0";        
            return false;
        }
        if (yts <= 0) {
            errorMessage = "BlackInputs->Years to settlement are <= 0";        
            return false;
        }
        if (sd <= 0) {
            errorMessage = "BlackInputs->Standard Deviation is <= 0";        
            return false;
        }
        if (df <= 0) {
            errorMessage = "BlackInputs->Discount Factor is <= 0";        
            return false;
        }
        if (yts < yte) {
            errorMessage = "BlackInputs->yearsToSettlement < yearsToExpiry; option cannot settle before it expires";        
            return false;
        }
        errorMessage =  "BlackInputs->No errors";
        return true;
    }
   ...
   =======================================================================================*/
    void EuropeanOptionBlack76::setParameters(BlackInputs blackInputs)
    {
        setParameters(blackInputs.getForward(), blackInputs.getStrike(), blackInputs.getYearsToExpiry(), 
                      blackInputs.getYearsToSettlement(), blackInputs.getStandardDeviation(),
                      blackInputs.getDiscountFactor());
    ...
    {
        BlackInputs blackInputs = BlackInputs(forward, strike, yearsToExpiry, yearsToSettlement, standardDeviation, discountFactor);
        if (!blackInputs.isOK()) {
            errorMessage = "EuropeanOptionBlack76->" + blackInputs.getErrorMessage();        
            return false;
        }
