...
void BlackScholesTest::testErrorMessages() {
    ...
    string expectedErrorMessage = "EuropeanCallBlack76->EuropeanOptionBlack76->Forward is <= 0";
    BOOST_CHECK(call.getErrorMessage().compare(expectedErrorMessage) == 0);
    call.setForward(F);

    put.setStandardDeviation(0);
    BOOST_CHECK(put.isOK() == false);
    expectedErrorMessage = "EuropeanPutBlack76->EuropeanOptionBlack76->Standard Deviation is <= 0";
