...
void MathsFunctionsTest::testFunctions() {
   ...
   BOOST_CHECK(combination(a,b) == (factorial(a) / (factorial(b) * factorial(a-b))));
   b = 11;
   BOOST_CHECK(combination(a,b) == 0);
...
void MathsFunctionsTest::testLinearArrayInterpolator()
{
   ...
   BOOST_CHECK(boost::isnan<double>(interpWithoutExtrap.getRate(10.75)) == true);
}
